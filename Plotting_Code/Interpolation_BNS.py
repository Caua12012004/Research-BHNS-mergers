...
for ind_n, name_t in enumerate(req_times_list_names):
    ...
    if DEFAULT==True:
        ...
        if TIME=='tHubble':
            ...
            xstart, xend = np.log10(1), np.log10(70)


            x_separation_tGyr = create_adaptive_separationArray(xstart,xend, Niter_a1=10, Niter_a2=50, Niter_a3=30)
            x_separation_BNS_1=x_separation_tGyr

            x_eccentricities_tGyr = obtain_a_and_e_for_fixed_mergerTime(x_separation=x_separation_BNS_1, fixed_Tinsp=t_requested, Niter_a=20, Niter_e=50)
            x_eccentricities_BNS_1=x_eccentricities_tGyr

        elif TIME=='tenMyr':
            t_requested = (10*u.Myr).to(u.year)
            xstart, xend = np.log10(0.01), np.log10(100)


            x_separation_tTenMyr = create_adaptive_separationArray(xstart,xend, Niter_a1=10, Niter_a2=50, Niter_a3=30)
            x_separation_BNS_2=x_separation_tTenMyr

            x_eccentricities_tTenMyr = obtain_a_and_e_for_fixed_mergerTime(x_separation=x_separation_BNS_2, fixed_Tinsp=t_requested, Niter_a=20, Niter_e=50)
            x_eccentricities_BNS_2=x_eccentricities_tTenMyr        
...
print(len(x_eccentricities_BNS_2))