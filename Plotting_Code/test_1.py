...
for ind, model in enumerate(list_of_models):
    ...
    print(f"Model {model} array sizes: {len(rel_rates_max[25:31])} - {len(rel_rates_max[18:25])} - {len(rel_rates_max[10:18])} - {len(rel_rates_max[0:10])} - {len(rel_rates_min[25:31])} - {len(rel_rates_min[18:25])} -{len(rel_rates_min[10:18])} - {len(rel_rates_min[0:10])}")
...
axe[0].axhspan(1/Max_Reticulum_II_Mass, 1/Min_Reticulum_II_Mass, color='grey', alpha=0.5, lw=0, label = 'Reticulum II required rates')
axe[0].axhspan(1/Max_Tucana_III_Mass, 1/Min_Tucana_III_Mass, color='teal', alpha=0.6, lw=0, label = 'Tucana III required rates')
